...
namespace Account
{
        ...
        void free_account(AccountData *Account)
        {
                ...
                if (first_account == Account)
                        first_account = Account->next;
                if (last_account == Account)
                        last_account = Account->prev;
                delete Account;
        }
        ...
        CMDF do_showalts(CharData *ch, char *argument)
        {
                ...
                if (count == 0)
                {
                        send_to_char("&BT&zhat Account has no &Rcharacters&z linked to it.",
                                     ch);
                        return nullptr;
                }
                return nullptr;
        ...
        void fread_account(AccountData *Account, FILE *fp) // was void fread_account(AccountData * Account, FILE * fp) --- IGNORE ---
        {
                ...
                for (;;)
                {
                        ...
                        case 'P':
                                ...
                                break;
                        case 'Q':
                                if (!str_cmp(word, "Qpoints"))
                                {
                                        Account->qpoints = fread_number(fp);
                                        fMatch = true;
                                }
                                break;
                        case 'R':
                                if (!str_cmp(word, "RPpoints"))
                                {
                                        Account->rppoints = fread_number(fp);
                                        fMatch = true;
                                }
                                if (!str_cmp(word, "RPcurrent"))
                                {
                                        Account->rpcurrent = fread_number(fp);
                                        fMatch = true;
                                }
                                break;
                        }
        ...
        CMDF do_showaccounts(CharData *ch, char *argument)
        {
                ...
                if (victim && !(victim->pcdata == NULL || victim->pcdata->Account == NULL))
                {
                        Account = victim->pcdata->Account;
                        ch_printf(ch, "Account Name: %s\n\r", Account->name);
                        ch_printf(ch, "RP Points:    %d\n\r", Account->rppoints);
        ...
        CMDF do_switchchar(CharData *ch, char *argument)
        {
                ...
                if (d->Account == NULL)
                        return nullptr;
                if (auction && auction->item != nullptr && ((ch == auction->buyer) || (ch == auction->seller)))
                        set_char_color(AtPlain, ch);
                if (argument[0] == '\0')
                {
                        set_pager_color(AtPlain, ch);
                        send_to_pager("&RSyntax: &Gswitchchar &C<&ccharacter name&C>&w\n\r\n\r",
                                      ch);
                        ...
                        if (!IsImmortal(ch) && ch->in_room && !IsSet(ch->in_room->RoomFlags, RoomHotel) && !NotAuthed(ch))
                        {
